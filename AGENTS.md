...
- **Parsing prompt** in `llm_client.py` from PRD Section 6. Key behaviors:
  ...
  - Output: raw JSON only (`{"type","nominal","category","note","transaction_date"}`) — kategori pemasukan: gaji, freelance, bisnis, investasi, hadiah, lainnya; kategori pengeluaran: makanan, minuman, transportasi, belanja, hiburan, tagihan, kesehatan, investasi, lainnya
